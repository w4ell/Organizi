...
router.post("/create-user", async (req, res, next) => {
  try {
    ...
    const activationUrl = `https://partify.vercel.app/activation/${activationToken}`;

    try {
      await sendMail({
        email: user.email,
        subject: "Activez votre compte",
        message: `Salut ${user.name}, veuillez cliquer sur le lien pour activer votre compte: ${activationUrl}`,
      });
      res.status(201).json({
        success: true,
